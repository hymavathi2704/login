...
// ==============================
// 🔑 NEW: Check Client Review Eligibility
// ==============================
export const checkReviewEligibility = async (req, res) => {
    ...
    if (!clientId || req.user?.roles.includes('coach')) {
        // Only non-coach, logged-in users (clients) can write reviews
        return res.json({ isEligible: false, reason: 'Not authenticated as a client.' });
    }
    ...
    try {
        ...
        // Find if the client has *any* confirmed booking with this coach
        const hasConfirmedBooking = await Booking.findOne({
            where: { 
                clientId: clientId,
                // Status must be confirmed or completed (assuming confirmed means services rendered/scheduled)
                status: 'confirmed'
            },
            include: [{
                model: Session,
        ...
        const isEligible = !!hasConfirmedBooking;
        
        return res.json({ isEligible, reason: isEligible ? 'Client has confirmed bookings.' : 'No confirmed sessions found.' });
