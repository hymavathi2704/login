...
          
          // 🛑 NEW ACTION: LOG SUCCESS DATA INSTEAD OF REDIRECTING
          console.log("SUCCESS: Order Created! Payment Session ID:", paymentSessionId);
          console.log("--- Payment SDK Checkout Skipped for Testing ---");
          
          // 🛑 CRITICAL: COMMENT OUT THE REDIRECT LINE
...
          
          toast.success("Order created successfully! Check console for ID.");
          
...
          }
      } else {
          // This path handles the server failing to return the session ID
          throw new Error("Server failed to initiate payment session.");
      }
      
    } catch (err) {
      // If the backend threw an error (like 'authentication Failed'), it lands here.
